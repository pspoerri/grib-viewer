...
func (s *Server) contributors(composite string) []string {
	...
	for _, src := range s.Cfg.Sources {
		...
		if composite == "auto_eps" && s.Engine.MembersFor(src.ID, info.RunID, name) == 0 {
			continue
		}
...
// compositeModelDTO builds the union catalog for a composite pseudo-
// model: finest contributor wins per variable.
